...
const attachmentTypes = exports.AttachmentType;
/**
...
const captchaTypes = exports.CaptchaType;
...
const messageSources = exports.MessageSource;
(function (ResourceType) {
    ResourceType["USER"] = "user";
    ResourceType["GROUP"] = "group";
    ResourceType["APPLICATION"] = "application";
})(exports.ResourceType || (exports.ResourceType = {}));
const resourceTypes = exports.ResourceType;
...
const apiErrors = exports.APIErrorCode;
...
const uploadErrors = exports.UploadErrorCode;
...
})(exports.SnippetErrorCode || (exports.SnippetErrorCode = {}));
const snippetsErrors = exports.SnippetErrorCode;
...
const sharedErrors = exports.SharedErrorCode;
/**
 * VK Platforms
 */
...
class APIRequest {
    ...
    [util.inspect.custom](depth, options) {
        ...
        return `${options.stylize(name, 'special')} ${util.inspect(payload, options)}`;
    }
}
...
const { CAPTCHA_REQUIRED, USER_VALIDATION_REQUIRED, CONFIRMATION_REQUIRED } = exports.APIErrorCode;
...
const { CAPTCHA_REQUIRED: CAPTCHA_REQUIRED$1, TOO_MANY_REQUESTS, USER_VALIDATION_REQUIRED: USER_VALIDATION_REQUIRED$1 } = exports.APIErrorCode;
...
class API extends APIMethods {
    ...
    async handleError(request, error) {
        ...
        if (code === USER_VALIDATION_REQUIRED$1) {
            ...
            try {
                // @ts-ignore
                AccountVerification = (await new Promise(function (resolve) { resolve(_interopNamespace(require('@vk-io/authorization'))); })).AccountVerification;
            }
            catch (importError) {
                request.reject(error);
                return;
...
class Attachment {
    ...
    [util.inspect.custom](depth, options) {
        ...
        const payload = this.$filled
            ? `${util.inspect(customData, { ...options, compact: false })}`
            : '{}';
...
class ExternalAttachment {
    ...
    [util.inspect.custom](depth, options) {
        const { name } = this.constructor;
        const customData = this.toJSON();
        const payload = util.inspect(customData, { ...options, compact: false });
        return `${options.stylize(name, 'special')} ${payload}`;
    }
}

const { POLL } = exports.AttachmentType;
...
const { GIFT } = exports.AttachmentType;
...
const { WALL } = exports.AttachmentType;
...
const { PHOTO } = exports.AttachmentType;
...
const { LINK } = exports.AttachmentType;
...
const { AUDIO } = exports.AttachmentType;
...
const { VIDEO } = exports.AttachmentType;
...
const { STORY } = exports.AttachmentType;
...
const { MARKET } = exports.AttachmentType;
...
const { STICKER } = exports.AttachmentType;
...
const { GRAFFITI } = exports.AttachmentType;
...
const { DOCUMENT } = exports.AttachmentType;
/**
 * Types of documents
 */
...
const { WALL_REPLY } = exports.AttachmentType;
...
const { MARKET_ALBUM } = exports.AttachmentType;
...
const { AUDIO_MESSAGE } = exports.AttachmentType;
...
class Upload {
    ...
    constructor(vk) {
        this.vk = vk;
        this.graffiti = util.deprecate(params => (
        // @ts-ignore
...
const { APP_TOKEN_NOT_VALID, RESPONSE_SIZE_TOO_BIG } = exports.APIErrorCode;
const { EXECUTE_ERROR } = exports.CollectErrorCode;
class CollectStream extends stream.Readable {
    /**
     * Constructor
     */
...
class MessageReply {
    ...
    [util.inspect.custom](depth, options) {
        ...
        return `${options.stylize(name, 'special')} ${util.inspect(payload, { ...options, compact: false })}`;
    }
}
...
class WebhookTransport {
    ...
    async start({ path = '/', tls, host, port } = {}, next) {
        ...
        try {
            ...
            const listen = util.promisify(webhookServer.listen).bind(webhookServer);
    ...
    async stop() {
        ...
        if (this.webhookServer !== null) {
            const { webhookServer } = this;
            const close = util.promisify(webhookServer.close).bind(webhookServer);
            await close();
            this.webhookServer = null;
        }
    }
...
class Updates {
    ...
    async start({ webhook } = {}) {
        ...
        if (!this.vk.options.pollingGroupId) {
            try {
                ...
                if (error.code !== exports.APIErrorCode.WRONG_PARAMETER) {
                    throw error;
                }
    ...
    reloadMiddleware() {
        ...
        if (this.hearStack.length !== 0) {
            stack.push(
            // @ts-ignore
            middlewareIo.getOptionalMiddleware((context) => context.is('new_message') && !context.isEvent, middlewareIo.compose([
                ...this.hearStack,
...
const { INVALID_URL, INVALID_RESOURCE, RESOURCE_NOT_FOUND } = exports.SnippetErrorCode;
const numberRe = /^-?\d+$/;
...
class ResourceResolver {
    ...
    async resolveUrl(rawResourceUrl) {
        const resourceUrl = !hasProtocolRe.test(rawResourceUrl)
            ...
            : rawResourceUrl;
        const { pathname, search } = new url.URL(resourceUrl);
        if (pathname === '/') {
            throw new SnippetsError({
                code: INVALID_URL,
    ...
    async resolveScreenName(resource) {
        ...
        if (type === 'page') {
            return {
                id,
                type: exports.ResourceType.GROUP
            };
        }
        return { id, type };
    }
...
class Composer extends middlewareIo.Composer {
    /**
...
exports.default = VK;
exports.inspectCustomData = inspectCustomData;
